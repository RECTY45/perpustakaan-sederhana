...
// Query untuk mendapatkan data peminjaman beserta detail bukunya dan anggotanya
$sql = "SELECT peminjaman.*, buku.judul, anggota.nama FROM peminjaman 
...
if ($data) {
    ...
    $selected_book_title = $data['judul'];
    $id_anggota = $data['id_anggota'];
    $selected_anggota_name = $data['nama'];
    $pinjam_berapa_hari = $data['pinjam_berapa_hari'];
    $tanggal_peminjaman = $data['tanggal_peminjaman'];
    $tanggal_wajib_dikembalikan = $data['tanggal_wajib_dikembalikan'];
    $tanggal_kembali = $data['tanggal_kembali'];
    $status_peminjaman = $data['status_peminjaman'];
    $pinjam_berapa_hari = $data['pinjam_berapa_hari'];
    $perpanjangan_peminjaman = $data['perpanjangan_peminjaman'];
    $status_denda = $data['status_denda'];
    $catatan_perpustakaan = $data['catatan_perpustakaan'];
} else {
    $id_buku = '';
    $selected_book_title = '';
    $id_anggota = '';
    $selected_anggota_name = '';
    $pinjam_berapa_hari = '';
}
?>
                    ...
                    <div class="card">
                        <div class="card-header d-flex justify-content-between">
                            ...
                            <div class="header-title">
                                <h4 class="card-title">Sunting Peminjaman</h4>
                            </div>

                            <a class="btn btn-primary rounded-pill" href="../peminjaman">
                                <span class="btn-inner">
                                    <svg class="icon-32" width="32" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <path opacity="0.4" d="M15.7975 10.8096L19.4967 10.4824C20.3269 10.4824 21 11.1621 21 12.0003C21 12.8386 20.3269 13.5182 19.4967 13.5182L15.7975 13.1911C15.1463 13.1911 14.6183 12.6579 14.6183 12.0003C14.6183 11.3416 15.1463 10.8096 15.7975 10.8096Z" fill="currentColor"></path>
                                        <path d="M3.37534 10.8698C3.43316 10.8115 3.64915 10.5647 3.85206 10.3598C5.03568 9.07656 8.12619 6.97815 9.7429 6.33596C9.98835 6.23352 10.6091 6.01542 10.9418 6C11.2592 6 11.5625 6.0738 11.8516 6.2192C12.2127 6.42299 12.5007 6.74463 12.66 7.12355C12.7614 7.38572 12.9207 8.17331 12.9207 8.18763C13.0789 9.04792 13.165 10.4469 13.165 11.9934C13.165 13.465 13.0789 14.8067 12.949 15.6813C12.9349 15.6967 12.7756 16.6738 12.6021 17.0086C12.2847 17.6211 11.664 18 10.9996 18H10.9418C10.5087 17.9857 9.5989 17.6057 9.5989 17.5924C8.06837 16.9502 5.05096 14.9532 3.83788 13.6258C3.83788 13.6258 3.49534 13.2844 3.34698 13.0718C3.11571 12.7656 3.00007 12.3866 3.00007 12.0077C3.00007 11.5847 3.12989 11.1915 3.37534 10.8698Z" fill="currentColor"></path>
                                    </svg>
                                </span>
                                Kembali
                            </a>
                        ...
                        <div class="card-body">
                            <p>Sunting data peminjaman buku.</p>
                            <form action="../peminjaman/proses_edit.php" method="POST">
                            <input type="hidden" name="id" value="<?php echo $data['id_peminjaman']; ?>">
                                <div class="row">
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            <label class="form-label" for="nama_anggota_display">Nama Anggota</label>
                                            <input type="text" class="form-control" id="nama_anggota_display" name="nama_anggota_display" value="<?php echo $selected_anggota_name; ?>" disabled>
                                            <input type="hidden" id="nama_anggota" name="id_anggota" value="<?php echo $id_anggota; ?>">
                                        </div>
                                    </div>
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            <label class="form-label" for="judul_buku">Judul Buku</label>
                                            <input type="text" class="form-control" id="judul_buku_display" name="judul_buku_display" value="<?php echo $selected_book_title; ?>" disabled>
                                            <input type="hidden" id="judul_buku" name="id_buku" value="<?php echo $id_buku; ?>">
                                        </div>
                                    </div>
                                </div>
                                <div class="row">
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            ...
                                            <input type="date" class="form-control" id="tanggal_peminjaman" value="<?php echo $tanggal_peminjaman?>" name="tanggal_peminjaman">
                                        </div>
                                    </div>
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            <label class="form-label" for="tanggal_wajib_dikembalikan">Tanggal Wajib Dikembalikan</label>
                                            <input type="date" class="form-control" id="tanggal_wajib_dikembalikan" value="<?php echo $tanggal_wajib_dikembalikan ?>" name="tanggal_wajib_dikembalikan">
                                        </div>
                                    </div>
                                </div>
                                <div class="row">
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            ...
                                            <input type="date" class="form-control" id="tanggal_kembali" value="<?php $tanggal_kembali ?>" name="tanggal_kembali">
                                        </div>
                                    </div>
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            <label class="form-label" for="status_peminjaman">Status Peminjaman</label>
                                            <select class="form-control" id="status_peminjaman" name="status_peminjaman">
                                                <option value="<?php echo $status_peminjaman ?>"><?php echo $status_peminjaman ?></option>
                                                <option value="">Status Peminjaman </option>
                                                <option value="Dipinjam">Dipinjam</option>
                                                <option value="Dikembalikan Tepat Waktu">Dikembalikan Tepat Waktu</option>
                                ...
                                <div class="row">
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            <label class="form-label" for="pinjam_berapa_hari">Pinjam Berapa Hari</label>
                                            <input type="text" class="form-control" id="pinjam_berapa_hari" name="pinjam_berapa_hari" value="<?php echo $pinjam_berapa_hari; ?>" placeholder="Masukkan Durasi Pinjam dalam Hari...">
                                        </div>

                                    </div>
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            <label class="form-label" for="perpanjangan_peminjaman">Perpanjangan Peminjaman</label>
                                            <input type="text" class="form-control" id="perpanjangan_peminjaman" name="perpanjangan_peminjaman" value="<?php echo $perpanjangan_peminjaman ?>" placeholder="Masukkan Jumlah Perpanjangan dalam Hari...">
                                        </div>
                                    </div>
                                </div>
                                <div class="row">
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            <label class="form-label" for="status_denda">Status Denda</label>
                                            <select class="form-control" id="status_denda" name="status_denda">
                                                <option value="<?php echo $status_denda ?>"><?php echo $status_denda ?></option>
                                                <option value="">Pilih Status Denda</option>
                                                <option value="Tidak Denda">Tidak Denda</option>
                                                <option value="Kena Denda">Kena Denda</option>
                                            </select>
                                        </div>
                                    </div>
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            <label class="form-label" for="catatan_perpustakaan">Catatan Perpustakaan</label>
                                            <textarea class="form-control" id="catatan_perpustakaan" name="catatan_perpustakaan"placeholder="Masukkan Catatan Perpustakaan..."><?php echo $catatan_perpustakaan ?> </textarea>
                                        </div>
                                    </div>
                                </div>
